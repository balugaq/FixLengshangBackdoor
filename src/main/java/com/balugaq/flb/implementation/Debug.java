...
public class Debug {
    ...
    public static void debug(String message) {
        if (FixLengshangBackdoor.getConfigManager().isDebug()) {
    ...
    public static void log(@NotNull String message) {
        init();
        plugin.getServer().getConsoleSender().sendMessage("[" + FixLengshangBackdoor.getInstance().getName() + "] " + ChatColor.translateAlternateColorCodes('&', message));
    }
    ...
    public static void init() {
        if (plugin == null) {
            plugin = FixLengshangBackdoor.getInstance();
        }
    }
    ...
    public static void trace(@NotNull Throwable e, @Nullable String doing, @Nullable Integer code) {
        init();
        plugin.getLogger().severe("DO NOT REPORT THIS ERROR TO " + FixLengshangBackdoor.getInstance().getName() + " DEVELOPERS!!! THIS IS NOT A " + FixLengshangBackdoor.getInstance() + " BUG!");
        if (code != null) {
            plugin.getLogger().severe("Error code: " + code);
        }
        plugin.getLogger().severe("If you are sure that this is a " + FixLengshangBackdoor.getInstance().getName() + " bug, please report to " + FixLengshangBackdoor.getInstance().getBugTrackerURL());
        if (doing != null) {
